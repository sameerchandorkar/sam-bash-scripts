...
while read server; do
  ...
  # Loop over each sa file in the /var/log/sa directory and generate reports
#  ssh "$server" 'for FILE in /var/log/sa/sa[0-9][0-9]; do
#    DATE=$(basename "$FILE" | sed "s/sa//")
#    echo "Generating report for $DATE"
#    sar -u -f $FILE >> "$SERVER_OUTPUT_DIR/${DATE}.cpuutilization.txt"
#  done'
