...
while read -r server_name; do
    ...
    while read -r folder_name_suffix; do
        ...
        # Create the "server_output" folder if it doesn't already exist
        if [ ! -d "server_output" ]; then
            mkdir server_output
        fi

        # Create the folder inside the "server_output" folder
        folder_path="server_output/$folder_name"
        mkdir "$folder_path"
        ...
        while read -r file; do
            ...
            scp "$server_name:$file" "$folder_path" >> /dev/null
