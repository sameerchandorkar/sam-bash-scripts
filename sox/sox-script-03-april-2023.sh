...
while read -r server_name; do
    ...
    while read -r folder_name_suffix; do
        ...
        folder_name="server_output/${server_name}_${current_date}_${folder_name_suffix}"
        mkdir "$folder_name"
        ...
        # Execute the grep command on the remote server and copy the output to the folder
        ssh "$server_name" "grep -E 'root' /var/log/secure" > "$folder_name/${server_name}_secure"
